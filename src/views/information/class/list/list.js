import { articleCateList, articleCateDelete } from '@/api/site'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      articleCateList(parse).then(res => {
    ...
    // 删除
    del(e) {
      this.$prompt(`确认删除分类' ${e.title} '吗?`, '确认删除', {
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(({ value }) => {
        articleCateDelete({ id: e.id, note: value }).then(res => {
          if (res.result.isSuccess) {
