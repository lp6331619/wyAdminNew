import { resourceCloudNodeCpuMemList } from '@/api/resource'
...
export default {
  ...
  computed: {
    getDType() {
      return this.$route.query.detail ? this.$route.query.detail : 'cpuMem'
    }
  },
  created() {
  ...
  methods: {
    // 获取 schema prepare
    getRule(type) {
      resourceCloudNodeCpuMemList({}, type).then(res => {
        type === 'prepare'
    ...
    getList() {
      ...
      resourceCloudNodeCpuMemList(parse).then(res => {
