import { resourceCloudLineList } from '@/api/resource'
...
export default {
  ...
  data() {
    return {
      ...
      // 权限
      operatePrivBox: {
        search: 'resource:cloud:node:line:list',
  ...
  computed: {
    getDType() {
      return this.$route.query.detail ? this.$route.query.detail : 'line'
    }
  },
  created() {
  ...
  methods: {
    // 获取 schema prepare
    getRule(type) {
      resourceCloudLineList({}, type).then(res => {
        type === 'prepare'
    ...
    getList() {
      ...
      resourceCloudLineList(parse).then(res => {
