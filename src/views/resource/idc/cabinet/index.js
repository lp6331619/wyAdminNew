import { resourceIdcCabinetList, resourceIdcCabinetDelete } from '@/api/resource'
...
export default {
  ...
  data() {
    return {
      ...
      // 权限
      operatePrivBox: {
        search: 'resource:idc:cabinet:list',
  ...
  methods: {
    // 获取 schema prepare
    getRule(type) {
      resourceIdcCabinetList({}, type).then(res => {
    ...
    getList() {
      ...
      resourceIdcCabinetList(parse, '_withSchema').then(res => {
    ...
    // 删除
    del(e) {
      this.$prompt(`确认删除机柜 ${e.name}`, '确认删除', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputPlaceholder: '备注'
      }).then(({ value }) => {
        resourceIdcCabinetDelete({ id: e.id, note: e.value }).then(res => {
          if (res.result.isSuccess) {
