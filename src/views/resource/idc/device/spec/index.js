import { resourceIdcDeviceSpecList, resourceIdcDeviceSpecDelete } from '@/api/resource'
...
export default {
  ...
  data() {
    return {
      ...
      // 权限
      operatePrivBox: {
        search: 'resource:idc:device:spec:list',
  ...
  methods: {
    // 获取 schema prepare
    getRule(type) {
      resourceIdcDeviceSpecList({}, type).then(res => {
    ...
    getList() {
      ...
      resourceIdcDeviceSpecList(parse, '_withSchema').then(res => {
        if (res.result.isSuccess) {
          this.listData = res
    ...
    // 删除
    del(e) {
      this.$prompt(`确认删除设备规格 ${e.name}`, '确认删除', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputPlaceholder: '备注'
      }).then(({ value }) => {
        resourceIdcDeviceSpecDelete({ id: e.id, note: e.value }).then(res => {
          if (res.result.isSuccess) {
