import { resourceIdcDeviceOemList, resourceIdcDeviceOemDelete } from '@/api/resource'
...
export default {
  ...
  methods: {
    // 获取 schema prepare
    getRule(type) {
      resourceIdcDeviceOemList({}, type).then(res => {
    ...
    getList() {
      ...
      resourceIdcDeviceOemList(parse, '_withSchema').then(res => {
        if (res.result.isSuccess) {
          this.listData = res
    ...
    // 删除
    del(e) {
      this.$prompt(`确认删除设备品牌 ${e.name}`, '确认删除', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputPlaceholder: '备注'
      }).then(({ value }) => {
        resourceIdcDeviceOemDelete({ id: e.id, note: e.value }).then(res => {
          if (res.result.isSuccess) {
